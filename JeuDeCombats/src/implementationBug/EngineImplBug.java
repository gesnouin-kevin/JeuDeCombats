...
public class EngineImplBug implements EngineService{
	...
	public void init(int h, int w, int s, PlayerService p1, PlayerService p2) {
		...
		p1.init(this, 20); // add bug 20 not 0 or 1
		p2.init(this, 1);
		...
		this.player[0].getFightCharacter().setRectangleHitboxService(new RectangleHitboxContract(new RectangleHitboxImplBug()));
		this.player[1].getFightCharacter().setRectangleHitboxService(new RectangleHitboxContract(new RectangleHitboxImplBug()));
	...
	public void step() {
		if(!this.getPlayer(0).getFightCharacter().isDead() && !this.getPlayer(1).getFightCharacter().isDead()){
			...
			//this.commandPlayer2 = Command.NEUTRAL; // add bug command must be neutral
		}
		else{
			this.gameOver = true;
			this.commandPlayer1 = Command.NEUTRAL;
